...
class mod_assign_renderer extends plugin_renderer_base {
    ...
    public function render_assign_submission_status(assign_submission_status $status) {
        $o = '';
        $o .= $this->output->container_start('submissionstatustable');
        //$o .= $this->output->heading(get_string('submissionstatusheading', 'assign') . ":", 3);i
	$time = time();
	$o .= "<br>";
	if(!(!$status->submissionsenabled && $status->submission->status == ASSIGN_SUBMISSION_STATUS_NEW)){
		//$o .= "submissions!";
	...
	$editable = true;
	if ($three_days_later < $time && ! (strcmp($sub_status, 'new') == 0)){
            $editable = false;
	...
	//each group is one of the exercise preference options.
        ...
        //$previous = $_GET["prev"];
        //$o .=         $previous;
	// ##This comment is DEPRECATED, NEW 'PREVIOUS' IMPLEMENTATION IS WORKING If you want to add a button to go to the previous assignment,
	// you would add a parameter to the link url below and then if $previous
	// exists, you would add a link to it
	$role_sql = "SELECT * FROM mdl_role_assignments WHERE userid = $USER->id";
        $role = $DB->get_record_sql($role_sql);
        if($role->roleid != 9 && $role->roleid !=10 && $role->roleid !=12 && $role->roleid !=13){
                $role->roleid = 9;
	}
...
AND (dest.groupd LIKE  (CASE WHEN dest.groupd LIKE '%yes%' OR dest.groupd LIKE '%no%' THEN '%$data->groupd%' WHEN dest.groupd LIKE '%' THEN '%' END))
        AND (dest.groupe LIKE (CASE WHEN dest.groupe LIKE '%yes%' OR dest.groupe LIKE '%no%' THEN '%$data->groupe%' WHEN dest.groupe LIKE '%' THEN '%' END)) AND roleid= $role->roleid";
...
AND (dest.groupd LIKE  (CASE WHEN dest.groupd LIKE '%yes%' OR dest.groupd LIKE '%no%' THEN '%$data->groupd%' WHEN dest.groupd LIKE '%' THEN '%' END))
        AND (dest.groupe LIKE (CASE WHEN dest.groupe LIKE '%yes%' OR dest.groupe LIKE '%no%' THEN '%$data->groupe%' WHEN dest.groupe LIKE '%' THEN '%' END))  AND roleid =$role->roleid";
	$prev = $DB->get_record_sql($prev_sql);
	//print_r($sql);
	//print_r("<br>");
	//print_r($prev_sql);
	//echo "<br>";
	...
	if($a && $a->destination > 0){
                $o .= '<h3><a style="background:linear-gradient(to bottom, #bddbfa 5%, #b3d3da 100%); padding:0.2em;  border-radius:8px; border: 3px solid black;" href="https://mindful.rc.fas.harvard.edu/mod/'. $module .'/view.php?id='. $a->destination .'"> Please click here to continue</a></h3>';
	}
	//if there's no available next page, then it goes in this block, which links users to the "Thank you" page
		 ...
		 if($role->roleid == 12 || $role->roleid ==13){
			if($user->icq >= 21){
		...
		$o .= '<h3><a style="background:linear-gradient(to bottom, #bddbfa 5%, #b3d3da 100%); padding:0.2em;  border-radius:8px; border: 3px solid black;" href="https://mindful.rc.fas.harvard.edu/mod/page/view.php?id=' .$thank_you_pageid. '"> Please click here to continue</a></h3>';
		 }
...
$o .= '<br><h3><a style="background:linear-gradient(to bottom, #bddbfa 5%, #b3d3da 100%); padding:0.2em;  border-radius:8px; border: 3px solid black;" href="http://mindful.rc.fas.harvard.edu/mod/'. $module2 .'/view.php?id='. $prev->origin .'"> Click here to return to the previous activity</a></h3>';
	...
	//$previous = $_GET["prev"];
	//$o .= 	$test;
	// If you want to add a button to go to the previous assignment, you would add a parameter to the link url below and then if $previous
	// exists, you would add a link to it
	$sql = "SELECT * FROM mdl_assign_destination as dest WHERE origin = $submissionplugin->coursemoduleid AND (dest.groupa LIKE '% 'OR dest.groupa LIKE '%$groupa%') AND (dest.groupb LIKE '%$data->groupb%' OR dest.groupb LIKE '% ')
	AND (dest.groupc LIKE '%$data->groupc%' OR dest.groupc LIKE '% ') AND (dest.groupd LIKE '%$data->groupd%' OR dest.groupd LIKE '% ')
	AND (dest.groupe LIKE '%$data->groupe%' OR dest.groupe LIKE '% ')";
//$sql = "SELECT * FROM mdl_user WHERE username ='$username';";
	//$o .= $sql;
	$a = $DB->get_record_sql($sql);
	$module = $a->is_assign ? 'assign' : 'page'; 
	if($a){
		$o .= '<h2><a style="background-color:white;  border-radius:8px; padding=2em 2em;" href="http://mindful.rc.fas.harvard.edu/mod/'. $module .'/view.php?id='. $a->destination .'"> Please continue to next activity here</a></h2>';
	}
	else{
	$o .= '<h2><a style="background-color:white;  border-radius:8px; padding=2em 2em;" href="http://mindful.rc.fas.harvard.edu/mod/page/view.php?id=166"> Please continue to next activity here</a></h2>';
	}
	//$o .= print_r($a);
	//$o .= $USER->id;
	 */	
