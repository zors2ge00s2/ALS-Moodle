...
class condition extends \core_availability\condition {
     ...
     * @param int $newdate
     * @return int relative date.
     * Edited to instead use 6 AM (EST, 11 AM GMT) as the time for the next day's exercises.
     */
    private function fixcalc ($olddate, $newdate) {
        if ($this->relativedwm > 1) {
            $arr1 = getdate($olddate);
            $arr2 = getdate($newdate);
	    return mktime( /*$arr2['hours']*/ 6, /*$arr2['minutes']*/ 0, /*$arr2['seconds']*/ 0, $arr1['mon'], $arr1['mday'], $arr1['year']);
        }
        return $olddate;
    }
}
