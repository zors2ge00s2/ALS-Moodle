...
class core_course_renderer extends plugin_renderer_base {
    ...
    public function frontpage_section1() {
        global $SITE, $USER;
	global $DB; $CFG;
	//this is where stuff would be inserted
	...
	//Redirects users who haven't assigned their preferences yet, specifically if they are Patients (role id 9), and therefore
	//the redirect is to course id 2 for pref.php (which is the mindfulness course
	if($role->roleid == 9){
		if(!groups_is_member(1, $USER->id) && !groups_is_member(2, $USER->id)){;
			echo '<meta http-equiv="refresh" content="0; URL=https://mindful.rc.fas.harvard.edu/lib/pref.php?id=2" />';
			die();
		}
	}
...
AND (dest.groupd LIKE  (CASE WHEN dest.groupd LIKE '%yes%' OR dest.groupd LIKE '%no%' THEN '%$data->groupd%' WHEN dest.groupd LIKE '%' THEN '%' END))
        AND (dest.groupe LIKE (CASE WHEN dest.groupe LIKE '%yes%' OR dest.groupe LIKE '%no%' THEN '%$data->groupe%' WHEN dest.groupe LIKE '%' THEN '%' END))  AND dest.day = $days AND dest.roleid= $role->roleid";
	//echo $sql;		
	//print_r ($role->roleid);
