...
var COMPONENTNAME = 'atto_media',
    ...
    CSS = {
        ...
        MINUTE_INPUT: 'atto_media_minute_entry',
        SECOND_INPUT: 'atto_media_second_entry',
        ...
        QUESTIONS: 'atto_media_questions',
        QUESTIONS_CLASS: 'atto_media_questions_class',
    ...
    SELECTORS = {
        ...
        MINUTE_INPUT: '.' + CSS.MINUTE_INPUT,
        SECOND_INPUT: '.' + CSS.SECOND_INPUT,
    ...
    TEMPLATES = {
        ...
        TAB_PANES: {
            ...
            VIDEO: '' +
                ...
                '</div>' +
                '</fieldset>' +
                '<fieldset class="collapsible collapsed" id="{{elementid}}_video-questions">' +
                '<input name="mform_isexpanded_{{elementid}}_video-questions" type="hidden">' +
                '<legend class="ftoggler">Questions</legend>' +
                '<div class="fcontainer">' +
                '{{renderPartial "form_components.questions" context=this id=CSS.VIDEO}}' +
                '</div>' +
                '</fieldset>' +
        ...
        FORM_COMPONENTS: {
            ...
            DISPLAY_OPTIONS: '' +
                ...
                '<div>',
            QUESTIONS: '' +
                '<div class="{{CSS.QUESTIONS_CLASS}}">' +
                '<div class="mb-1">' +
                '<label for="{{id}}_media-questions">Enter your question: </label>' +
                '<textarea class="form-control fullwidth {{CSS.QUESTIONS}}" id="{{id}}_media-questions"' +
                'size="125"></textarea>' +
                '</div>' +
                '<div class="mb-1">' +
                '<label>Time to place question at</label>' +
                '<div class="form-inline {{CSS.POSTER_SIZE}}">' +
                '<label class="accesshide">{{get_string "videowidth" component}}</label>' +
                '<input type="text" class="form-control mr-1 {{CSS.MINUTE_INPUT}} input-mini" size="2"/>' +
                ':' +
                '<label class="accesshide">{{get_string "videoheight" component}}</label>' +
                '<input type="text" class="form-control ml-1 {{CSS.SECOND_INPUT}} input-mini" size="2"/>' +
                '</div>' +
                '</div>' +
                '<div class="clearfix"></div>' +
                '<div>',
        ...
        HTML_MEDIA: {
            VIDEO: '' +
                ...
                '</div>' +
                "{{#questions}}" +
                "<div id=media_question data-question= '{{../questions}}'</div> {{/questions}}" +
                "{{#minute}} <div id=media_minute data-minute ='{{../minute}}' </div>{{/minute}} " +
                "{{#second}} <div id=media_second data-second ='{{../second}}' </div> {{/second}}" +
                "<script> src='https://ajax.googleapis.com/ajax/libs/jquery/3.5.1/jquery.min.js'></script>" +
                "<script>" +
                "var checkExist = setInterval(function() {" +
                "var doc = document.getElementsByClassName(\"video-js\")[0];" +
                "if (videojs.getPlayer(doc.id).id_.length > 0) {" +
                "execute();" +
                "clearInterval(checkExist);" +
                "}" +
                "}, 100);" +
                "function execute() {" +
                "var box = document.querySelector('[style=\"max-width:400px;\"]');" +
                "box.style[\"max-width\"] = \"75%\";" +
                "var cls = document.getElementById('page-mod-page-view') || document.getElementById('page-mod-assign-view');" +
                "cls = cls.className;" +
                "var t = cls.split(' ').find(el => el.substring(0, 4) === 'cmid');" +
                "cls = parseInt(t.substring(t.indexOf('-') + 1));" +
                "var doc = document.getElementsByClassName(\"video-js\")[0];" +
                "var myPlayer = videojs.getPlayer(doc.id);" +
                "myPlayer.fluid(true);" +
                "var title = doc.title;" +
                "var furthestwatched = 0;" +
                "var played = 0;" +
                "var watched = new Boolean(false);" +
                "myPlayer.ready(function() {" +
                //"question = {{../questions}}" +
                //"if(question){" +
                "var q= document.getElementById('media_question');" +
                "if (q){" +
                "this.on('timeupdate', addQuestion);" +
                "this.on('seeked', function(){ myPlayer.pause(); addQuestion(); });" +
                "function addQuestion() {" +
                "var minutes = document.getElementById('media_minute');" +
                "var seconds = document.getElementById('media_second');" +
                "if(minutes){ minutes= parseInt(minutes.getAttribute('data-minute') || 0);}" +
                "else{ minutes = 0;}" +
                "if(seconds){ seconds = parseInt(seconds.getAttribute('data-second') || 0);}" +
                "else{ seconds = 0;}" +
                "seconds = (minutes * 60) + seconds;" +
                "if (this.currentTime() > seconds && watched == false) {" +
                "watched = new Boolean(true);" +
                "myPlayer.pause();" +
                "var player = this, options ={};" +
                "options.content = 'In the  modal';" +
                "options.label = 'the label';" +
                "var width = Math.round(myPlayer.currentWidth() * 0.5) + \"px\";" +
                "var height = Math.round(myPlayer.currentHeight() * 0.5) + \"px\";" +
                "var newElement = document.createElement('div');" +
                "var question = q.getAttribute('data-question');" +
                "newElement.innerHTML = \"<form> <label style='font-size:3em;' for='response_text'>\" + question + \":" +
                "</label><br><textarea style='border-radius:12px; width:\" + width + \"; height:\" + height + \"; font-size:2em;'" +
                "id='response_text' name='response_text' rows='6' cols='50'></textarea><br><br><input type='submit'" +
                "style='border-radius:8px; font-size:2.5em;'  value='Submit your response'></form>\";" +
                ...
                "var resp = $('textarea[name=response_text]').val();" +
                "require(['core/ajax']," +
                "function(ajax) {" +
                "ajax.call([{" +
                "methodname: 'mod_page_submit_video_response'," +
                "method: 'POST'," +
                "args: {" +
                "pageid: cls," +
                "" +
                ...
                "videoresponse: resp" +
                "}," +
                "done: console.log(\"recorded video response.\")," +
                "fail: console.log(\"\")," +
                "}]);" +
                "});" +
                "myModal.close();" +
                ...
                "console.log('paused');" +
                "if (myPlayer.played().length > played) {" +
                "var thisstart = Math.round(myPlayer.played().start(played));" +
                "var thisend = Math.round(myPlayer.played().end(played));" +
                "if (thisstart == thisend) {" +
                "return;}" +
                "played++;" +
                "furthestwatched = thisend;" +
                "} else {" +
                "var thisstart = furthestwatched;" +
                "var thisend = Math.round(myPlayer.played().end(played - 1));" +
                "if (thisstart == thisend) {" +
                "return;" +
                "}" +
                "furthestwatched = thisend;" +
                ...
                "function(ajax) {" +
                "ajax.call([{" +
                "methodname: 'mod_page_view_video'," +
                "method: 'POST'," +
                "args: {" +
                "pageid: cls," +
                "videoid: title," +
                ...
                "thisend: thisend" +
                "}," +
                "done: console.log('logged latest watch')," +
                "fail: console.log('')," +
                "}]);" +
                "});" +
                "})" +
...
Y.namespace('M.atto_media').Button = Y.Base.create('button', Y.M.editor_atto.EditorPlugin, [], {
    ...
    _applyMediumProperties: function(content, properties) {
        ...
        Y.Object.each(properties.tracks, function(value, key) {
            ...
            Y.Array.each(trackData.slice(1), function(track) {
                this._addTrackComponent(
                    tabPane.one(paneSelector + ' ' + SELECTORS.TRACK + ' .addcomponent'),
        ...
        if (mediumType === 'video') {
            ...
            tabPane.one(SELECTORS.QUESTIONS).set('value', properties.questions);
            tabPane.one(SELECTORS.MINUTE_INPUT).set('value', properties.minute);
            tabPane.one(SELECTORS.SECOND_INPUT).set('value', properties.second);
        }
    ...
    _getMediumProperties: function(medium) {
        ...
        return {
            ...
            questions: medium.getAttribute('questions'),
            minute: medium.getAttribute('minute'),
            second: medium.getAttribute('second'),
    ...
    _getMediaHTMLVideo: function(tab) {
        ...
        context.minute = tab.one(SELECTORS.MINUTE_INPUT).get('value') || false;
        context.second = tab.one(SELECTORS.SECOND_INPUT).get('value') || false;
        context.questions = tab.one(SELECTORS.QUESTIONS).get('value') || false;
