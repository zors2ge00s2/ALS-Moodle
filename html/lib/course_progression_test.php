<?php
		...
		foreach($users as $user){
			...
			echo "<br> Submissions?: ";
			echo $submissions_required ? 'true' : 'false';
			echo "<br>";
			...
			if($user->icq < 22){
				...
				if($user->skype == 2){
					...
					foreach($admin_ids as $admins){
						$admin_u = $DB->get_record("user", array('id'=>$admins));
						print_r($admin_u);
					//	echo email_to_user($admin_u, $admin_u, "User hasn't done exercises in 3 days!", "The user " . $user->firstname . ' has not progressed for 3 days in a row. Their email address is ' . $user->email); 
					}
				}
