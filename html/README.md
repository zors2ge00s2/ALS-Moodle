...
Note: internally, Moodle uses the term Assignment for the structure that we use to represent our exercises. An assignment is a page with a submission and grading options available, but the grading part has been hidden and when creating or editing an assignment there's always the option to leave no submission options.  

Appearance/Theming:  

The current theme is called Adaptable, named such for the ease with which you can customize a lot of the aspects of the theme. Part of it is a custom CSS section, which can be found in Site Administration > Appearance > Themes > Adaptable > Custom CSS & JS.  
...
The processing of all this happens in `/mod/assign/renderer.php`.
...
Database:  
The database is a standard docker mysql container, that is linked into the Moodle container. (Find notes on command used to `up` it initially). If the database is ever down, the first thing to try is `docker run DB`. 
...
SMTP email client: Currently using HUITs email client that they provided the credentials for. If migrating off harvard infrastructure will need a new SMTP email system, and to change it just search for SMTP in the search bar at the bottom of the navigation bar and change the credentials there.
